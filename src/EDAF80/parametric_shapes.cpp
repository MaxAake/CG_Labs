                              ...
                              unsigned int const vertical_split_count)
{
	...
	float const width_per_vert = width / horizontal_slice_vertices_count;
	float const height_per_vert = height / vertical_slice_vertices_count;
                                ...
                                unsigned int const latitude_split_count, bool bump)
{
	...
	auto const lat_slice_vertices_count = lat_slice_edges_count + 1u;
	auto const lon_slice_vertices_count = lon_slice_edges_count + 1u;
	auto const vertices_nb = lat_slice_vertices_count * lon_slice_vertices_count;
	...
	float const d_theta = glm::two_pi<float>() / (static_cast<float>(lat_slice_edges_count));
	float const d_phi = glm::pi<float>() / (static_cast<float>(lon_slice_edges_count));
	...
	// generate indices iteratively
	index = 0u;
	for (unsigned int i = 0u; i < lat_slice_edges_count; ++i)
	{
	...
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::normals), 3, GL_FLOAT, GL_TRUE, 0, reinterpret_cast<GLvoid const*>(normals_offset));
	...
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::tangents), 3, GL_FLOAT, GL_TRUE, 0, reinterpret_cast<GLvoid const*>(tangents_offset));

	glBufferSubData(GL_ARRAY_BUFFER, binormals_offset, binormals_size, static_cast<GLvoid const*>(binormals.data()));
	glEnableVertexAttribArray(static_cast<unsigned int>(bonobo::shader_bindings::binormals));
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::binormals), 3, GL_FLOAT, GL_TRUE, 0, reinterpret_cast<GLvoid const*>(binormals_offset));
